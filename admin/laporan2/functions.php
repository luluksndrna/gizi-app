...
function tambah($data){
	...
	$query="INSERT INTO laporan2 VALUES ('', '$id_pasien', '$waktu','$kamar','$diet','$tanggal','$kalori','$protein','$karbo','$lemak')";
...
function hapus($data){
	global $conn;
	$query="SELECT * FROM laporan2 WHERE id=$data";
	$sql=mysqli_query($conn,$query);
	$hapus=mysqli_fetch_assoc($sql);
	mysqli_query($conn,"DELETE FROM laporan2 WHERE id=$data");
	return mysqli_affected_rows($conn);
}
